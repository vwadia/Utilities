function [handlesToFig] = plotRastersTextinChunks(pathOut, raster, times, order, offsets, transcription, textParams, specificColour, isPDF, strctCells)
...
%     keyboard
    globalYl = findingGlobalYLim(psth, unique(order), order, 'AIC');

    for j = 1:numSubRasters
        if size(binPsth{j}, 1) > 1 && length(unique(binPsth{j}(2, :))) > 1
            stdshade5(binPsth{j}, 0.1, colors(j, :), binTimes, 2);
        else
    ...
    if numSubRasters == 1
        plot([binTimes(1) binTimes(end)], [mean(mean(psth)) mean(mean(psth))], '--k', 'LineWidth', 1.5);
...
%         end
    ...
    if size(binRaster{1}, 1) == 2 % single free recall raster
        yl(2) = 2;
...
%             ty.FontWeight = 'bold';
    ...
    textParams.supertitle = [textParams.suptitle '\_' num2str(bin)];
    ...
    filename = [pathOut filesep textParams.fold_name '_' filenum];
