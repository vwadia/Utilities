...
firingRates = firingRates - min(firingRates(:)) + 10; % how is this normalizing?  
...
if ~ifSimultaneousRecording
    trialNum = randi([5 E], [N S D]); % output [Neurons StimConds Decisions] random integers between 5 and E (why 5?)
else
...
% combines the last 3 dimensions into 1
% now we have neurons x stimuli*decisions*times
X = firingRatesAverage(:,:); 
X = bsxfun(@minus, X, mean(X,2)); % subtracting mean of each neuron
